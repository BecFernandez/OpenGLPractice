...
void FontComponent::Draw(const char * a_szToDraw)
{
	glm::vec3 centrePos = m_pGameObject->m_position;
	float curX = 0;
	...
	while(*currentLetter != '\0')
	{
		...
		//change the UVs
		SetSpriteUVCoords(thisChar.m_iX/(float)m_textureDimensions.x, 1.0 - 
			(thisChar.m_iY+ thisChar.m_iHeight)/(float)m_textureDimensions.y,
			(thisChar.m_iX + thisChar.m_iHeight)/(float)m_textureDimensions.x, 1.0 - (thisChar.m_iY )/(float)m_textureDimensions.y);
