...
GameScreen::GameScreen(SoundSystemClass* a_pSounds, GLSLProgram *a_pShaders) : Screen(a_pSounds, a_pShaders), 
	m_player(10, glm::vec3(800/2, 600/2, 0), glm::vec4(1.0f, 1.0f, 1.0f, 1.0f), 51, 86, "ship.png", &m_playerBullets, *m_pSounds)
{
	m_enemies = new Enemy*[3];
	for(int i = 0; i < 3; i++)
	{
		m_enemies[i] = new Enemy(50, glm::vec3(rand()%800, rand()%600, 0), glm::vec4(1.0f, 1.0f, 1.0f, 1.0f), 51, 86, "cylon.png", &m_enemyBullets, *m_pSounds);
	}
}
	...
	*/
	...
	for(unsigned int i = 0; i < m_playerBullets.size(); ++i)
	{
		...
		for(unsigned int j = 0; j < 3; ++j)
		{
			...
			{
				m_enemies[j]->Hurt(m_player.GetPower());
				//remove cylon
				//add explosion
